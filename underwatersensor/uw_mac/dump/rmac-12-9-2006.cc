...
printf("rmac:scheduleACKRev: node %d and node %d t1=%f the time is %f\n",index_, receiver_addr,t1,NOW);
    ...
    if (period_table[i].node_addr==receiver)
	  ...
	  {
    s_period=dt-(l-2*max_short_packet_transmissiontime);
    t1=s_period+Random::uniform()*(duration_-2*max_short_packet_transmissiontime); 
          }
	else{ 
        s_period=PeriodInterval_+dt-(l-2*max_short_packet_transmissiontime);
        t1=s_period+Random::uniform()*(duration_-2*max_short_packet_transmissiontime); 
	}
 ...
 printf("rmac:scheduleACKRev: node %d and node %d t1=%f the time is %f\n",index_, receiver,t1,NOW);  
	...
	double end_time=st+duration+2*max_large_packet_transmissiontime;
        double available_period=PeriodInterval_-elapsed_time;

        l=l-2*max_short_packet_transmissiontime;
      
        while (s_period<=end_time+l)s_period+=PeriodInterval_;
         
        while (available_period<(s_period-l))available_period+=PeriodInterval_;
        
        SetStartTime(ack_rev_pt,st,available_period);

 printf("rmac:scheduleACKRev: node %d set mac_recv after %f  and set the next time period is %f at %f\n",index_,st,available_period,NOW);
...
double 
RMac::CheckNextAvailableTable(int addr)
{
  double t=0.0;
  for (int i=0;i<TABLE_SIZE;i++){
    if(next_available_table[i].node_addr==addr) 
      t=next_available_table[i].required_time;
  }
  return t;
}
...
void 
RMac::ProcessNextAvailableTable()
{
  for (int i=0;i<TABLE_SIZE;i++){
    next_available_table[i].required_time-=PeriodInterval_;
    if(next_available_table[i].required_time<0.00000000001) 
      next_available_table[i].required_time=0.0; 
  }
}

void 
RMac::InsertNextAvailableTable(int node_addr,double next_time)
  {

    bool newid=true;
    for (int i=0;i<TABLE_SIZE;i++)
      {
	if(next_available_table[i].node_addr==node_addr) 
	  {
	    newid=false;
           next_available_table[i].required_time=next_time;
	  }
      }
    ...
    if (newid) 
      {
	int i=0;
	while(next_available_table[i].node_addr!=-1)i++;
        next_available_table[i].node_addr=node_addr;
        next_available_table[i].required_time=next_time; 
 ...
 printf("rmac MakeReservation: node %d lock txbuffer \n",index_);
  ...
  // printf("rmac:Makereservation node %d num=%d size=%d, encoding_efficiency=%f phy=%d and bit_rate= %f\n",index_,num, large_packet_size_,encoding_efficiency_,PhyOverhead_, bit_rate_);  
	  ...
	  double elapsed_time=NOW-cycle_start_time;
          ...
          double next_time=CheckNextAvailableTable(receiver_addr);
	
          
	  next_time=next_time-elapsed_time;
printf("makerreservation!! node_%d get the next available period of node %d is %f now time is %f\n",index_,receiver_addr,next_time,NOW);
        
           next_time=next_time-l+3*max_short_packet_transmissiontime;  

	  if(next_time>=0) { 
double t2=Random::uniform()*(duration_-max_short_packet_transmissiontime); 
printf("rmac:Makereservation node %d will send a reservation after %f now time is %f\n",index_,next_time+t2,NOW);    
         Scheduler& s=Scheduler::instance();
        s.schedule(&reserve_handler, (Event*) pkt,(next_time+t2));
	  }
	  else{
...
RMac::ProcessACKRevPacket(Packet* pkt)
{
  printf("RMac:ProcessACKRevPacket: node %d at time %f\n",index_,NOW);
    ...
    double next_time=ackrevh->interval;
   
    double  l=CheckLatency(short_latency_table,sender_addr);
    double  it=st-l;
    double interval=next_time-l;
    double elapsedtime=NOW-cycle_start_time;
  printf("rmac:ProcessAckRevPacket:node %d I get the ACK REV packet interval is %f and duration=%f next time is %f present time is %f\n",index_, it,dt,elapsedtime+interval,NOW);     
    ...
    InsertNextAvailableTable(sender_addr,interval+elapsedtime);
