...
/* 
the V_SHIFT is not used to update position, therefore, the ts_ in V_Shift 
   is set 0
*/

  if(t1> positionUpdateTime){
   calculatePosition(pkt);
   positionUpdateTime=t1;
  }
...
printf("vectrobasedforward node %d: this packet has been terminated\n",here_.addr_);
 ...
 if (p){
  ...
  if(msg_type==FLOODING){
      receiving_buffer.AddNewPacket(packet);
      if(receiving_buffer.bufferSize()==1) forwardPacket();   
    return;
}
...
void VectorbasedforwardAgent::forwardPacket()
{
    if(recving_buffer.IsEmpty()) return;
    Packet* packet=receiving_buffer.Dehead();
    if(!packet)
{
 printf("what the hell! I can not get the packet from a non-empty buffer\n");
    return;
}
