...
VectorbasedforwardAgent::VectorbasedforwardAgent() : Agent(PT_UWVB),delayhandler(this),void_avoidance_buffer(15),void_avoidance_handler(this),flooding_handler(this),flooding_forward_handler(this),mini_distance(20.0),mini_threshold(2.0),control_packet_size(32*8), receiving_buffer(10)
{
...
double d3=(UnderwaterChannel::Transmit_distance())/SPEED_OF_SOUND_IN_WATER;   
     ...
     printf("Vectorbasedforward(%d) set the timer %f at %f\n",THIS_NODE.addr_,DELAY*sqrt(mini_threshold)+d3*4,NOW);  
        ...
        Packet* pt=pkt->copy();
        double c=DELAY*sqrt(mini_threshold)+d3*4;
       if(!pt){
...
*/
      ...
      printf("vectorbased: node (%d) is still worth forwarding the data packet c=%d and tdelay=%f \n", here_.addr_,c,tdelay);  
           ...
           double d3=(UnderwaterChannel::Transmit_distance())/SPEED_OF_SOUND_IN_WATER;    
           set_shift_timer(pkt,(sqrt(mini_threshold)*DELAY*2+d3*4)); 
               MACprepare(pkt);
               MACsend(pkt,0);      
		 }
               else{
