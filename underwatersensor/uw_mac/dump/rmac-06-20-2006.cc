...
RMac::SendLargeAckND()
{
  ...
  hdr_rmac* ackndh = HDR_RMAC(pkt);
      ...
      time_record* t1=new time_record[TABLE_SIZE];     

      hdr_cmn*  cmh = HDR_CMN(pkt);
      

        ackndh->ptype = P_LACKND;
       ackndh->pk_num = num_send;
      ackndh->sender_addr=index_;
      num_send++;
 ...
 // timestamp
  ...
  memcpy((void*)pkt->accessdata(),(void*)arrival_table,sizeof(arrival_table));
 

 
  /*
   printf("rmac SendND:node(%d) send Large ACKND  %f\n", ackndh->sender_addr,NOW);
...
printf("rmac:ProcessAckRevPacket: node %d this ACKREV is for me\n",index_); 
        ...
        num_data=0;
        double  l2=CheckLatency(large_latency_table,sender_addr);
        double  it1=it-l2+2*max_large_packet_transmissiontime;
...
RMac::ProcessDataPacket(Packet* pkt)
{
  ...
  printf("rmac:ProcessDataPacket: node %d get data packet type is %d send up num=%d \n",index_,vbh->mess_type,num);
...
RMac::ProcessLargeACKNDPacket(Packet* pkt)
{
  // printf("rmac:ProcessLargeACKNDPacket: node %d\n",index_);
    ...
    memcpy((void*)arrival_t,(void*)pkt->accessdata(),sizeof(time_record[TABLE_SIZE]));

  printf("rmac :node(%d) process large ACKNDPacket at %f\n", index_, NOW);

  
  /*
				if(pkt->accessdata()){
         time_record* t1=new time_record[TABLE_SIZE];
         memcpy((void*)t1,(void*)pkt->accessdata(),sizeof(time_record[TABLE_SIZE]));
          for(int i=0;i<TABLE_SIZE;i++){
       printf("rmac node %d and arrival time %f\n", t1[i].node_addr,t1[i].arrival_time);	
	  }
	  delete t1;
				}
  */
...
RMac::RecvProcess(Packet* pkt){
  ...
  if(dst==MAC_BROADCAST){
    ...
    if (ptype==P_LACKND) ProcessLargeACKNDPacket(pkt); 
