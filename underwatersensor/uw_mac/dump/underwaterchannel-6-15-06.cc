...
static const char rcsid[] =
    "@(#) $Header: /nfs/jade/vint/CVSROOT/ns-2/mac/channel.cc,v 1.41 2003/12/23 17:36:34 haldar Exp $ (UCB)";
#endif
...
#include "underwatersensor/uw_common/uwvb_header.h" // added by peng xie
//#include "underwatersensor/uw_mac/cubepropagation.h" //added by peng xie
...
double UnderwaterChannel::distCST_ =20;
...
UnderwaterChannel::sendUp(Packet* p, Phy *tifp)
{
  //	 printf("underwaterchannel sendup\n");
		 ...
		 for (i=0; i < numAffectedNodes; i++) {
		   ...
		   double d1=distance(tnode,rnode);
		    ...
		    if((rnode == tnode)||(d1>distCST_))
			{
			 ...
			 /*
			   	struct hdr_cmn *hdr1 = HDR_CMN(newp);
                                struct hdr_uwvb* hdr2=HDR_UWVB(newp);
                               
				if(newp->accessdata()){
         time_record* t1=new time_record[TABLE_SIZE];
         memcpy((void*)t1,(void*)newp->accessdata(),sizeof(t1));
          for(int i=0;i<TABLE_SIZE;i++){
       printf("underwatermac node %d and arrival time %f\n", t1[i].node_addr,t1[i].arrival_time);	
	  } 
	...
	//	hdr1->direction()= hdr_cmn::UP;	 

				//   printf("underwaterchannel :the new packet type is %d and %d \n",hdr1->ptype_, hdr2->mess_type);	
...
UnderwaterChannel::calculatePosition(Node *sender,Node* receiver, Packet* p)
{
	...
	dz=r1->Z()-s1->Z();
        ...
        info->dz=dz;
	if(p->accessdata())
	  memcpy(p->accessdata(),(unsigned char*) info,sizeof(uw_extra_info));
