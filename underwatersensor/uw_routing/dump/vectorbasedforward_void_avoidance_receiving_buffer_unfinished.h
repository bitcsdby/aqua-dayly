...
class UWVoidAvoidTimer: public TimerHandler{
public:
UWVoidAvoidTimer(VectorbasedforwardAgent * a):TimerHandler(){a_=a;}
 ns_addr_t source;
...
class UWFloodingTimer: public TimerHandler{
public:
UWFloodingTimer(VectorbasedforwardAgent * a):TimerHandler(){a_=a;}
 ns_addr_t source_addr;
 unsigned int pkt_num;
protected :
virtual void expire(Event* e);
  VectorbasedforwardAgent * a_;
...
class UWFloodingForwardTimer: public TimerHandler{
public:
UWFloodingForwardTimer(VectorbasedforwardAgent * a):TimerHandler(){a_=a;}
Packet* packet;
protected :
virtual void expire(Event* e);
  VectorbasedforwardAgent * a_;
...
class VectorbasedforwardAgent : public Agent {
 ...
 protected:
  ...
  double  positionUpdateTime=0.0; // timestamp of the packet used to updated position
  ...
  UWVoidAvoidTimer void_avoidance_timer;
  UWFloodingTimer  flooding_timer;
  UWFloodingForwardTimer flooding_forward_timer;
...
// the width is used to test if the node is close enough to the path specified by the packet  
  ...
  void process_flooding_timeout(ns_addr_t,int);
...
//  Packet *prepare_message(unsigned int dtype, ns_addr_t to_addr, int msg_type);
  ...
  friend class UWVoidAvoidTimer;
  friend class UWFloodingTimer;
  friend class UWFloodingForwardTimer;
};
