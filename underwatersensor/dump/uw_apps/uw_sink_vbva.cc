...
UW_VBVA_SinkAgent::UW_VBVA_SinkAgent() : Agent(PT_UWVB), data_type_(0), 
...
void UW_VBVA_Sink::source_deny(ns_addr_t id, double x, double y, double z)
{
...
void UW_VBVA_Sink::bcast_interest()
{
...
int UW_VBVA_Sink::command(int argc, const char*const* argv)
{
...
void UW_VBVA_Sink::recv(Packet* pkt, Handler*)
{
    hdr_uwvb* vbh = HDR_UWVB(pkt);
...
void UW_VBVA_Sink::reset()
{
}


void UW_VBVA_Sink:: set_addr(ns_addr_t address)
{
  here_=address;
}


int UW_VBVA_Sink:: get_pk_count()
{
  return pk_count;
}


void UW_VBVA_Sink:: incr_pk_count()
{
  pk_count++;
}


Packet * UW_VBVA_Sink:: create_packet()
{
  Packet *pkt = allocpkt();
