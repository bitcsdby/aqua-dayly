...
CTSHandler::CTSHandler(TMac* p):mac_(p){}
...
TMac::TMac():UnderwaterMac(),short_nd_handler(this),short_acknd_window_handler(this), status_handler(this), phaseone_handler(this),acknd_handler(this), phasetwo_handler(this), phasethree_handler(this), wakeup_handler(this),timeout_handler(this),transmission_handler(this),ackdata_handler(this), rts_timeout_handler(this),backoff_handler(this),silence_handler(this),ack_handler(this),poweroff_handler(this),rts_handler(this),cts_handler(this)/*,rts_silence_handler(this)*/
{
 ...
 for(int i=0;i<TABLE_SIZE;i++){
  ...
  //interval_ND_ACKND=Latency_window_-ND_window_-ACKND_window_;
   ...
   max_propagationTime=TransmissionRange_/1500;
...
printf("\n. ..ReStart node %d re-start up at %f...\n\n",index_,NOW); 
 ...
 printf("\n. ..WakeUp node %d re-start up  at %f  and the number of packet is %d...\n\n",
...
printf("tmac SendRTS: node %d send RTS\n",index_);
  ...
  double dt=3.1*l+max_propagationTime+du*2.5;      
  ...
  printf("TxRTS, node %d is at %f\n",index_,NOW);
	...
	if(mac_status==TMAC_SILENCE)
     ...
     // printf("broadcast %d Tx Idle set timer at %f tx is %f\n",node_->nodeid(),NOW,txtime);
            ...
            printf("TxRTS, node %d Tx RTS at %f and timeout is %f\n",index_,NOW,t);
            status_handler.SetStatus(IDLE);
            Scheduler& s=Scheduler::instance();
            s.schedule(&status_handler,&status_event,txtime);
...
TMac::ProcessCTSPacket(Packet* pkt)
{
      ...
      if(t>0)
	{
	  if(mac_status=TMAC_SILENCE)
          {
	    if(last_silenceTime>=t+NOW) return;
	    else 
            {
            Scheduler& s=Scheduler::instance();
...
printf("tmac:ProcessACKDATAPacket: node %d process ACKDATA packets at time %f duration_=%f\n",index_,NOW,duration_);
  ...
  printf("tmac:ProcessACKDATAPacket: node %d received the butmap is:\n",index_);
...
TMac::ProcessRTSPacket(Packet* pkt)
{
    ...
    {
      printf("TMAC:ProcessRTS: node %d: I am not the intended receiver, do nothing \n", index_);
      return;
    }
    ...
    {  
      ...
      s.schedule(&ack_handler,&ack_event,dt);
  ...
  hdr_tmac* ctsh = HDR_TMAC(p); 
       ...
       for (int i=0;i<MAXIMUM_BUFFER;i++) bit_map[i]=0;
         ...
         if(IDLE==status){
            ...
            printf("SendCTS, node %d is in idle at %f\n",index_,NOW);
            status_handler.SetStatus(IDLE);
            Scheduler& s=Scheduler::instance();
	    ...
	    //   s.schedule(&ackdata_handler,&ack_event,t1);
  ...
  if(RECV==status)
    { 
     ...
     /*
      ...
      cmh->ts_=NOW;
      sendDown(p);
      printf("TxRTS, node %d is in recv at %f\n",index_,NOW);
      status_handler.SetStatus(IDLE);
      Scheduler& s=Scheduler::instance();
      s.schedule(&status_handler,&status_event,txtime);
      return;
...
{
 printf("tmac:ProcessDataPacket: node %d get data packet\n",index_);
  ...
  if(dst!=index_) 
{
 printf("tmac:ProcessDataPacket: node %d  this is not my packet\n",index_);
return;
}
 ...
 if (SEND==status)
  ...
  if (txbuffer.IsEnd()) {
   ...
   // double t=2.1*max_propagationTime+transmission_time_error_+dt;  
     double t=2.1*max_propagationTime+2.5*dt;  
...
{
   printf("tmac:node %d  gets a packet at  %f\n",index_,NOW);
   hdr_cmn* cmh=HDR_CMN(pkt);
