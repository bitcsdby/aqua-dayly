...
RMac::ScheduleACKREV(int receiver, double duration)
{
  ...
  while ((period_table[i].node_addr!=-1)&&(i<TABLE_SIZE))
      ...
      double elapsed_time=NOW-cycle_start_time;
       ...
       if (dt>0) dt=dt-PeriodInterval_;

 if (dt<=0){
     start_time=dt+duration_;
    if (start_time>=(l-3*max_short_packet_transmissiontime))
{
      delay=start_time-(l-3*max_short_packet_transmissiontime);
     double t1=0;
    if((-dt)>=max_short_packet_transmissiontime) t1=duration_+dt-max_short_packet_transmissiontime;
       else  t1=duration_-2*max_short_packet_transmissiontime;
       offset1= Random::uniform()*t1;  
}
    else {
      double limit=l-3*max_short_packet_transmissiontime;
             start_time=dt+PeriodInterval_;
      while(start_time+duration_<limit)
        start_time+=PeriodInterval_;

       dt=start_time;
       delay=start_time-(l-3*max_short_packet_transmissiontime);
       double t1=0;
       t1=duration_-2*max_short_packet_transmissiontime;
       offset1= Random::uniform()*t1;  
    }
...
printf("rmac:scheduleACKRev: node %d and node %d delay=%f the time is %f\n",index_, receiver_addr,delay,NOW);
       
       InsertACKRevLink(ackrev,delay);   
       Scheduler& s=Scheduler::instance();
       s.schedule(&ackrev_handler, (Event*) ackrev,delay);

       if(delay>last_time) last_time=delay;
	  }
	i++;
      }

printf("rmac:scheduleACKRev: node %d  last_time=%f the time is %f\n",index_,last_time,NOW);
    ...
    if (period_table[i].node_addr==receiver)
      ...
      elapsed_time=NOW-cycle_start_time;
       ...
       if (dt>0) dt=dt-PeriodInterval_;
        ...
        last_time=last_time+ACKRevInterval_; 
        double delay=0;
        double offset1=0;
        double start_time=0.0;

        double next_start=0.0;
        if ((PeriodInterval_-elapsed_time)>=last_time) 
	  next_start=0;
	else {
	  next_start=PeriodInterval_-elapsed_time;
	  while (next_start+PeriodInterval_<last_time) 
	    next_start+=PeriodInterval_;       
	}

	if (next_start<0.0000001){
            dt=dt-elapsed_time;
            start_time=dt+duration_;
       if (start_time>=(l-3*max_short_packet_transmissiontime))
{
    ...
    if((-dt)>=max_short_packet_transmissiontime) t1=duration_+dt-max_short_packet_transmissiontime;
      ...
      start_time=dt+PeriodInterval_;
      while(start_time<(l-3*max_short_packet_transmissiontime))
        start_time+=PeriodInterval_;
       ...
       offset1= Random::uniform()*t1;  
            ...
            start_time=dt+duration_;
	    if(start_time>(last_time-next_start-(l-2*max_short_packet_transmissiontime)+max_short_packet_transmissiontime))
 {
    delay=start_time-(l-3*max_short_packet_transmissiontime);
    double t1=start_time-(delay-next_start)-max_short_packet_transmissiontime;
       offset1= Random::uniform()*t1;    
 }
 else {
      start_time=dt+PeriodInterval_;
      while((start_time+duration_)<(last_time-next_start-l+3*max_short_packet_transmissiontime))
        start_time+=PeriodInterval_;
      double t1=0;
      if(start_time>=last_time-next_start) delay=start_time;
      else delay=last_time-next_start;

      t1=start_time+duration_-max_short_packet_transmissiontime-delay;
      delay=delay-(l-2*max_short_packet_transmissiontime);
       offset1= Random::uniform()*t1;  
       delay=delay+offset1;
 }
    ...
    printf("rmac:scheduleACKRev: node %d and node %d t1=%f the time is %f\n",index_, receiver,delay,NOW);  
        Packet* ackrevpacket=GenerateACKRev(receiver,receiver,duration);
        InsertACKRevLink(ackrevpacket,delay);   

        Scheduler& s=Scheduler::instance();
        s.schedule(&ackrev_handler, (Event*) ackrevpacket,delay+offset1); 

        double st=delay+2.0*l; 
...
printf("rmac MakeReservation: node %d MakeReservation \n",index_);
 ...
 printf("rmac MakeReservation: node %d lock txbuffer \n",index_);
  ...
  double duration=num*(((large_packet_size_*encoding_efficiency_+PhyOverhead_)/bit_rate_)+transmission_time_error_);
...
RMac::ProcessACKRevPacket(Packet* pkt)
{
  ...
  printf("rmac:ProcessAckRevPacket:node %d I get the ACK REV packet interval is %f and duration=%f present time is %f\n",index_, it,dt,NOW);     
    ...
    if(it<0) 
     printf("rmac:ProcessAckRevPacket: node %d the notification is too short\n",index_);
