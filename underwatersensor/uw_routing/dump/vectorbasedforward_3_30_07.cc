...
printf("vectrobasedforward node %d: this packet has been terminated\n",here_.addr_);
 ...
 p=void_avoidance_buffer.DeQueue(source,pkt_num);
 if (p){
 ...
 else printf("vectorbased node %d can not find the copy in my buffer\n",here_.addr_);
 Packet::free(packet);
  } 
 return;
...
double d3=(UnderwaterChannel::Transmit_distance())/SPEED_OF_SOUND_IN_WATER;
	...
	if(!pt){
       printf("vectorbasedforward(%d): the packet doesn't exist \n ",here_.addr_);
	}
