...
void UWPkt_Hash_Table::delete_hash(hdr_uwvb * vbh)
{
    Tcl_HashEntry *entryPtr;
...
void UWPkt_Hash_Table::delete_hash(ns_addr_t source, unsigned int pkt_num)
{
    Tcl_HashEntry *entryPtr;
...
{
  a_->process_flooding_forward_timeout((Packet*)e);
}
...
VectorbasedforwardAgent::VectorbasedforwardAgent() : Agent(PT_UWVB),delayhandler(this),void_avoidance_buffer(15),void_avoidance_handler(this),flooding_handler(this),flooding_forward_handler(this),mini_distance(20.0),mini_threshold(1.5), receiving_buffer(10)
{
...
neighborhood *hashPtr= PktTable.GetHash(vbh->sender_id, vbh->pk_num);
	...
	// printf("vectrobasedforward node %d: already forward no. %d packet\n",here_.addr_,vbh->pk_num); 
        Packet::free(packet);
	return;
    }

      hashPtr= CenterPktTable.GetHash(vbh->sender_id, vbh->pk_num);
 ...
 if(!p){
     //printf("vectorbased node %d: this packet has been used for other purpose\n",here_.addr_);
  Packet::free(packet);
    return;
 }
 ...
 if (tp){
     ...
     vbh->forward_agent_id=forwarder;   

     ConsiderNew(tp);   
      }
 else printf("vectorbased node %d can not get the packet copy \n",here_.addr_);
 Packet::free(packet);
 return;
...
// printf("vectrobasedforward node %d: receives a flooding packet from %d\n",here_.addr_,vbh->forward_agent_id.addr_); 
  ...
  if(msg_type==DATA){
    ...
    if (hashPtr) {       
      PktTable.put_in_hash(vbh,&sp,&ep,&fp);
       printf("vectrobasedforward node !!%d: this is duplicate packet %d from the forwarder %d sp.x=%f sp.y=%f and sp.z=%f fp.x=%f fp.y=%f and fp.z=%f\n",here_.addr_,vbh->pk_num,vbh->forward_agent_id.addr_,sp.x,sp.y,sp.z,fp.x,fp.y,fp.z); 
        Packet::free(packet);
	return;
    }
...
void VectorbasedforwardAgent::processFloodingPacket(Packet* pkt)
{
  ...
  ns_addr_t   from_nodeID, target_id, sender_id;
  int num=vbh->pk_num;
  ...
  p1.z=vbh->info.fz;    
	 ...
	 PktTable.delete_hash(vbh); // clear the records of the same packets
         PktTable.put_in_hash(vbh);
       double delay=calculateFloodingDesirableness(pkt);

      printf("Vectorbasedforward: %d set the flooding forward timer and delay is %f\n", here_.addr_, delay);
       Scheduler& s=Scheduler::instance();
       s.schedule(&flooding_forward_handler,(Event*)pkt,delay);
	}
	 return;
...
double d3=(UnderwaterChannel::Transmit_distance())/SPEED_OF_SOUND_IN_WATER;   
       ...
       if(!pt){
 printf("Vectorbasedforward(%d): can not find the packet in the buffer\n ",here_.addr_);
 return;
       }
       else {
 printf("Vectorbasedforward(%d): source sets flooding timer\n ",here_.addr_);
       Scheduler& s=Scheduler::instance();
       s.schedule(&flooding_handler,(Event*)pt,c);
       }
	 ...
	 if (IsCloseEnough(pkt)){
               ...
               PktTable.delete_hash(vbh);
               PktTable.put_in_hash(vbh,&sp,&ep,&fp);  

//	Packet* tpkt=void_avoidance_buffer.DeQueue(source,pkt_num); // avoid duplication
...
double d3=(UnderwaterChannel::Transmit_distance())/SPEED_OF_SOUND_IN_WATER;   
 double d4=Random::uniform()*JITTER;    
	   set_delaytimer(pkt,(sqrt(delay)*DELAY+d2*2+d3+d4));
	  }
	  else { 
       // put the data packet into its buffer to wait for void-avoidance use
// printf("vectorbased node(%d) is not  close to the vector\n",THIS_NODE.addr_);
...
void VectorbasedforwardAgent::set_delaytimer(Packet* pkt, double c){
 printf("vectorbased node(%d) is setting the timer %f at %f\n",THIS_NODE.addr_,c,NOW);
      Scheduler& s=Scheduler::instance();
      s.schedule(&delayhandler,(Event*)pkt,c);
}
...
void VectorbasedforwardAgent::set_shift_timer(Packet* pkt, double c){
 ...
 Packet* p=generateVectorShiftPacket(&source, pkt_num,&s_position,&t_position);
    printf ("vectorbasedforward (%d): sets void_avoidance timer\n ",here_.addr_);
       Scheduler& s=Scheduler::instance();
...
void VectorbasedforwardAgent::process_flooding_timeout(Packet* pkt)
{  
printf ("vectorbasedforward (%d): process flooding timeout at %f\n ",here_.addr_,NOW);
  if(!pkt) {
 printf ("vectorbasedforward %d: process flooding timeout packet is null\n ",here_.addr_);
 return;
  }
  ...
  hdr_uwvb* vbh = HDR_UWVB(pkt);
   ...
   unsigned int pkt_num=vbh->pk_num;
    ...
    int num_of_forwarder;
      ...
      neighborhood *terminationhashPtr= DataTerminationPktTable.GetHash(source, pkt_num);
     if(terminationhashPtr){
    printf ("vectorbasedforward(%d): The packet is already terminated!\n ",here_.addr_);
      Packet::free(pkt);
  return;
     }
/*
neighborhood *centerhashPtr= CenterPktTable.GetHash(source,pkt_num);
 if(centerhashPtr)
 {
  printf ("vectorbasedforward(%d): The packet is already sent as slef-centered!\n ",here_.addr_);
  Packet::free(pkt);
  return;
}
*/
...
{
printf ("vectorbasedforward(%d): flooding forward timer expires! at %f\n ",here_.addr_,NOW);
 if(!pkt) printf ("vectorbasedforward(%d): the packet doesn't exist \n ",here_.addr_);
 ...
 if(centerhashPtr)
 {
printf ("vectorbasedforward(%d): The packet is already sent as slef-centered,, to set flooding timer!\n ",here_.addr_);

         double d3=(UnderwaterChannel::Transmit_distance())/SPEED_OF_SOUND_IN_WATER;
	 // double c=DELAY*sqrt(mini_threshold)*2+d3*2;
            double d4=Random::uniform()*JITTER;    	    
           double c=DELAY*sqrt(mini_threshold)+JITTER+d3*2+d4;

	
               Packet* pt=pkt->copy();
	if(!pt) printf("vectorbasedforward(%d): can not get copy of the packet \n ",here_.addr_);
	else {
 printf("vectorbasedforward(%d): set flooding timer\n ",here_.addr_);
      Scheduler& s=Scheduler::instance();
       s.schedule(&flooding_handler,(Event*)pt,c);
	}
  Packet::free(pkt);
  ...
  if(!hashPtr){
     ...
     CenterPktTable.put_in_hash(vbh);
         ...
         double d3=(UnderwaterChannel::Transmit_distance())/SPEED_OF_SOUND_IN_WATER;
            double d4=Random::uniform()*JITTER;    	    
           double c=DELAY*sqrt(mini_threshold)*2+d3*2+d4;

	
               Packet* pt=pkt->copy();


	       // printf("vectorbasedforward(%d): before lookupcopy \n ",here_.addr_);
	      //  Packet* pt=void_avoidance_buffer.LookupCopy(source,pkt_num); 
	       // printf("vectorbasedforward(%d): after lookupcopy \n ",here_.addr_);
 ...
 printf("vectorbasedforward(%d): set flooding timer\n ",here_.addr_);
   ...
   printf ("vectorbasedforward(%d): sends out the slef-centered packet \n ",here_.addr_);
    MACprepare(pkt);
    MACsend(pkt,0);
       return;
    }
  ...
  double dist2=sqrt((mp.x-fp.x)*(mp.x-fp.x)+(mp.y-fp.y)*(mp.y-fp.y)+(mp.z-fp.z)*(mp.z-fp.z));
   ...
   //  double c=DELAY*sqrt(mini_threshold)*2+d3*2;
	...
	if(!pt) printf("vectorbasedforward(%d):  can not get the packet \n ",here_.addr_);   
	else {
   printf("vectorbasedforward(%d):  set flooding timer \n ",here_.addr_);   
       Scheduler& s=Scheduler::instance();
       s.schedule(&flooding_handler,(Event*)pt,c);
...
*/
		if(c>0) c--; // delete my first packet record    
...
//not necessary
 ...
 void VectorbasedforwardAgent::sendFloodingPacket(Packet* pkt){
     ...
     printf ("vectorbasedforward(%d): sends the flooding packet at %f !\n ",here_.addr_, NOW);
     MACprepare(pkt);    
     MACsend(pkt,0);
     return;
 }
...
void VectorbasedforwardAgent::process_void_avoidance_timeout(Packet* pkt)
{
    hdr_uwvb* vbh=HDR_UWVB(pkt);
    ns_addr_t source=vbh->sender_id;
    unsigned int pkt_num=vbh->pk_num;
    position s_p=vbh->original_source;
    position t_p;
...
printf ("vectorbasedforward(%d): The packet is already sent as slef-centered!\n ",here_.addr_);
  ...
  return;
    ...
    printf ("vectorbasedforward(%d): is void node\n ",here_.addr_);
        ...
        PktTable.delete_hash(source, pkt_num);// clear the tracks of this packet
        PktTable.put_in_hash(vbh); //keep the track of this packet
        MACsend(pkt, 0);       


	// Packet* pt=void_avoidance_buffer.DeQueue(source,pkt_num);
...
//        double c=DELAY*sqrt(mini_threshold)*2+d3*2;
 ...
 return;
	   ...
	   Packet* tmp=pt->copy();
       Scheduler& s=Scheduler::instance();
       s.schedule(&flooding_handler,(Event*)tmp,c);

      return;   
       }
    }
    else{
   printf ("vectorbasedforward(%d): is not a void node\n ",here_.addr_);
   Packet::free(pkt);
//  Packet* pt=void_avoidance_buffer.DeQueue(source,pkt_num);
   // if (pt) Packet::free(pt);
   return;
}
...
Packet* VectorbasedforwardAgent::generateVectorShiftPacket(const ns_addr_t* source,int pkt_num, const position* sp, const position* tp)
{
