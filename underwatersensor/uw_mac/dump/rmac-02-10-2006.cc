...
RMac::TxACKRev(Packet* pkt){
  ...
  if(IsACKREVWindowCovered(NOW)) {
     ...
     InsertReservedTimeTable(index_,0,4*PeriodInterval_);
  }
...
// receiver is the address of the data sender
...
RMac::ScheduleACKREV(int receiver, double duration, double offset)
{
  ...
  double elapsed_time=NOW-cycle_start_time;
	...
	// SetStartTime(ack_rev_pt,st,available_period);
      ...
      printf("rmac:scheduleACKRev: node %d offset time %f  and set the end time period is %f and available_period is %f  at %f\n", 
                index_,st,end_period,available_period,NOW);
...
{ 
  ...
  UnderwaterSensorNode* n=(UnderwaterSensorNode*) node_;
  if(n->CarrierSense()) carrier_sense=true;
    n->ResetCarrierSense();
}
...
RMac::DetermineSendingTime(int receiver_addr)
{
  ...
  double time_slot=max_short_packet_transmissiontime;
     ...
     offset_time=dt1+time_slot-elapsed_time-delay;
     while (offset_time<0) offset_time+=PeriodInterval_;        
    
   double dt2=dt1+duration_; // end of the time slot 
   ...
   while (period_table[i].difference<dt2)
    {
      if(period_table[i].difference>dt1)
	{
      double t=period_table[i].difference;
