...
double d3=(UnderwaterChannel::Transmit_distance())/SPEED_OF_SOUND_IN_WATER;   
     ...
     printf("Vectorbasedforward(%d) set the flooding timer %f at %f\n",THIS_NODE.addr_,DELAY*sqrt(mini_threshold)+d3*4,NOW);  
...
void VectorbasedforwardAgent::process_flooding_forward_timeout(Packet* pkt)
{
  ...
  if(!hashPtr){
     ...
     CenterPktTable.put_in_hash(vbh);
printf ("vectorbasedforward(%d): sends out the self-centered packet!\n ",here_.addr_);
// double d3=(UnderwaterChannel::Transmit_distance())/SPEED_OF_SOUND_IN_WATER;
...
//     s.schedule(&flooding_handler,(Event*)pt,c);
    ...
    forwarder_list= hashPtr->neighbor;       
    num_of_forwarder=hashPtr->number;
printf ("vectorbasedforward(%d): I have received the packets from %d nodes!\n ",here_.addr_,num_of_forwarder);
      position  sp,fp;
  ...
  double dist2=sqrt((mp.x-fp.x)*(mp.x-fp.x)+(mp.y-fp.y)*(mp.y-fp.y)+(mp.z-fp.z)*(mp.z-fp.z));
      ...
      CenterPktTable.put_in_hash(vbh);
printf ("vectorbasedforward(%d): I am still worth sending this self-centered packet!\n ",here_.addr_);
...
printf ("vectorbasedforward(%d): I am not worth sending this packet!\n ",here_.addr_);
		 Packet:free(pkt);
		 }
    return;
}
